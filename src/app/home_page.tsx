...
export default function Home() {
  ...
  // Save daily summary and trigger celebration when all tasks are done
  useEffect(() => {
  ...
  const handleUpdatePriority = useCallback(
    async (taskToUpdate: TimetableTask, newPriority: TimetableTask["priority"]) => {
      ...
      setTasks((prevTasks) =>
        prevTasks.map((task) =>
          task.id === taskToUpdate.id ? { ...task, priority: newPriority } : task,
        ),
      );
      ...
      if (error) {
        setLoadError("Failed to update priority.");
        setTasks((prevTasks) =>
          prevTasks.map((task) =>
  ...
  const handleReorderTasks = useCallback(
    async (reorderedTasks: TimetableTask[]) => {
      if (!supabase) return;
      setTasks(reorderedTasks);

      // Update order_index for each task in Supabase
